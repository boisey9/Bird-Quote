...
const nextButtonLabels: Record<RfqStep, string> = { 1: 'Next: Bus Selection', 2: 'Next: Seats & Options', 3: 'Next: Review', 4: 'Submit Quote Request' };

function HelpModal({ onClose }: { onClose: () => void }) {
  return (
    <div className="drawerBackdrop helpBackdrop" role="presentation">
      <aside className="helpModal" aria-label="RFQ help">
        <header className="drawerHeader"><div><small>RFQ V2 Help</small><h2>How the workflow works</h2><p>Dealer intake and internal workflow layer — not the final configurator.</p></div><button type="button" onClick={onClose}><X size={20} /></button></header>
        <div className="drawerContent"><section><h3>Dealer flow</h3><p>Submit structured RFQs in four steps: request info, bus selection, seats/options, and review.</p></section><section><h3>Seats and layouts</h3><p>Seat previews are reference only. Micro Bird validates final seating, engineering feasibility, and quote details.</p></section><section><h3>Documents</h3><p>Document metadata can still be captured later, but it is no longer a dedicated dealer step.</p></section></div>
      </aside>
    </div>
  );
}
...
export function App() {
  ...
  useEffect(() => { if (!user) return; if (!canAccessPage(user, page)) setPage(getDefaultPage(user)); }, [page, user]);
