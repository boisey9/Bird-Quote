...
export function MyRequestsPage({ user, onStartNew, onEditRequest, onDuplicateRequest }: MyRequestsPageProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchRfqRequests(scope), fetchRfqRows(scope)]).then(([items, rawRows]) => {
      if (!mounted) return;
      setRequests(items.length > 0 ? items : (isDealerView ? [] : mockRequests));
      setRows(rawRows);
      setLoadStatus(items.length > 0 ? (isDealerView ? 'Live RFQs loaded for this dealer account.' : 'Live RFQ data loaded from Neon.') : (isDealerView ? 'No RFQs found for this dealer account.' : 'No live RFQs yet. Showing sample requests.'));
    }).catch((error) => {
      if (!mounted) return;
      const message = error instanceof Error ? error.message : 'Unable to load RFQ requests.';
      setLoadStatus(isDealerView ? message : `${message} Showing sample requests.`);
  ...
  return (
    <section className="contentCard pageCard">
      ...
      <section className="panel tablePanel">
        ...
        {requests.length === 0 ? <EmptyState title="No RFQs yet" message="Start a new RFQ and it will appear here for editing, duplication, and status tracking." actionLabel="Start New RFQ" onAction={onStartNew} /> : <div className="requestTable">
          <div className="requestHead requestHeadWithActions"><span>RFQ</span><span>Customer</span><span>Configuration</span><span>Status</span><span>Owner</span><span>SLA</span><span>Last Update</span><span>Actions</span></div>
          {requests.map((request) => {
            const row = getRow(request.id);
            const canEdit = Boolean(isDealerView && row && canDealerEditRfq(row));
            return <div className="requestRow requestRowWithActions" key={request.id}><strong>{request.id}<small>{request.submittedDate}</small></strong><span>{request.finalCustomer}<small>{request.dealer}</small></span><span>{request.busType}<small>{request.chassis} • {request.wheelbase}</small></span><StatusBadge status={request.status} /><span>{request.owner}</span><span>{request.slaAge}</span><span>{request.lastUpdate}</span><span className="rowActionButtons">{isDealerView && row && <button type="button" className="btn btn-ghost btn-sm" disabled={!canEdit} onClick={() => onEditRequest?.(row)}>Edit</button>}{isDealerView && row && <button type="button" className="btn btn-ghost btn-sm" onClick={() => onDuplicateRequest?.(row)}>Duplicate</button>}</span></div>;
          })}
        </div>}
      </section>
    </section>
